      ...
      <div className="panel-body">
        {tab === 'props' && (<>
          ...
          <div className="prop-section">
            ...
            {engines.map(e => (
              <div key={e.name} className="prop-row">
                <span className="prop-label">{e.name}</span>
                <span><span className={`status-dot ${e.running ? 'green' : 'red'}`} />{e.running ? '運行' : '停止'}</span>
              </div>
            ))}
          </div>
        </>)}
        ...
        {tab === 'material' && (<>
          <div className="prop-section">
            ...
            {materials.map(m => (
              <div key={m.id} className={`layer-item ${activeMaterialId === m.id ? 'active' : ''}`} onClick={() => setActiveMaterial(m.id)}>
                <div className="color-swatch-sm" style={{ background: m.albedo }} />
        ...
        {tab === 'lod' && (<div className="prop-section">
          ...
          {lodLevels.map((l, i) => (
            <div key={i} className="prop-row" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 4 }}>
        ...
        {tab === 'load' && (<div className="prop-section">
          <div className="prop-section-title">負載分析 (FEA)</div>
          {loadAnalysis.result ? (<>
            <div className="prop-row">
              <span className="prop-label">總邊數</span>
              <span className="prop-value">{loadAnalysis.result.totalEdges}</span>
            </div>
            <div className="prop-row">
              <span className="prop-label">危險邊</span>
              <span className="prop-value" style={{ color: loadAnalysis.result.dangerCount > 0 ? 'var(--error)' : 'var(--success)' }}>
                {loadAnalysis.result.dangerCount}
              </span>
            </div>
            <div className="prop-row">
              <span className="prop-label">最大應力比</span>
              <span className="prop-value" style={{ color: loadAnalysis.result.maxStressRatio > 1 ? 'var(--error)' : loadAnalysis.result.maxStressRatio > 0.8 ? 'var(--warning)' : 'var(--success)' }}>
                {loadAnalysis.result.maxStressRatio.toFixed(3)}
              </span>
            </div>
            <div className="prop-row">
              <span className="prop-label">應力覆蓋</span>
              <span className="prop-value">{loadAnalysis.showStressOverlay ? '顯示中' : '隱藏'}</span>
            </div>
          </>) : (
            <div className="text-xs text-muted" style={{ padding: 8 }}>尚未執行 FEA 分析，請至右側負載分析面板操作</div>
          )}
        </div>)}
      </div>
    </div>
